...
document.addEventListener('DOMContentLoaded', function() {
    var compboxes = document.getElementsByClassName('compbox');
    var playerimgs = document.getElementsByClassName('playerimg');
  
    for (var i = 0; i < playerimgs.length; i++) {
      playerimgs[i].addEventListener('click', function() {
        var compbox = getEmptyCompBox(compboxes);
        if (compbox) {
          var imgSrc = this.getAttribute('src');
          var imgAlt = this.getAttribute('alt');
  
          if (!compbox.hasChildNodes()) {
            var imgElement = document.createElement('img');
            imgElement.src = imgSrc;
            imgElement.alt = imgAlt;
            imgElement.classList.add('selectedimg');
  
            imgElement.addEventListener('click', function() {
              this.parentNode.removeChild(this);
            });
  
            compbox.appendChild(imgElement);
          } else {
            compbox.innerHTML = '';
          }
        }
    ...
    function getEmptyCompBox(compboxes) {
      for (var i = 0; i < compboxes.length; i++) {
        if (!compboxes[i].hasChildNodes()) {
          return compboxes[i];
        }
      }
      return null;
    ...
    function getSelectedFileNames() {
        var selectedImgs = document.getElementsByClassName('selectedimg');
        var fileNames = [];
      
        for (var i = 0; i < selectedImgs.length; i++) {
          var imgSrc = selectedImgs[i].getAttribute('src');
          var fileNameWithExtension = imgSrc.substring(imgSrc.lastIndexOf('/') + 1);
    ...
    // Compare 버튼 클릭 이벤트 처리
    var cmpButton = document.getElementById('cmpbutton');
    cmpButton.addEventListener('click', function() {
      var dataStats = getSelectedFileNames();
  
      if (dataStats.length >= 2) {
        var player1 = dataStats[0];
        var player2 = dataStats[1];

        switch(player1){
            case 'anthony':
                var p1stat=[56,34.0,25.9,9.7,17.2,56.3,0.3,1.3,25.7];
                break;
            case 'booker':
                var p1stat=[11, 41.7, 33.7, 12.8, 21.9, 58.5, 2.8, 5.5, 50.8];
                break;
            case 'curry':
                var p1stat=[13, 37.9, 30.5, 11.2, 23.9, 46.6, 4.4, 12.1, 36.3];
                break;
            case 'fox':
                var p1stat=[38.5, 27.4, 10.0, 23.6, 42.4, 3.0, 9.0, 33.3];
                break;
            case 'giannis':
                var p1stat=[3, 30.5, 23.3, 9.3, 17.7, 52.8, 0.0, 1.0, 0.0];
                break;
            case 'harden':
                var p1stat=[11, 38.8, 20.3, 6.5, 16.6, 39.3, 2.8, 7.5, 37.8];
                break;
            case 'jimmy':
                var p1stat=[64, 33.4, 22.9, 7.5, 13.9, 53.9, 0.6, 1.6, 35.0];
                break;
            case 'joel':
                var p1stat=[9, 37.4, 23.7, 7.3, 17.0, 43.1, 0.6, 3.1, 17.9];
                break;
            case 'lebron':
                var p1stat=[55, 35.5, 28.9,  11.1, 22.2, 50.0, 2.2, 6.9, 32.1];
                break;
            case 'trae':
                var p1stat=[74, 36.4, 20.5, 8.3, 17.8, 46.4, 1.8, 5.2, 34.4];
                break;
        }

        switch(player2){
            case 'anthony':
                var p2stat=[56,34.0,25.9,9.7,17.2,56.3,0.3,1.3,25.7];
                break;
            case 'booker':
                var p2stat=[11, 41.7, 33.7, 12.8, 21.9, 58.5, 2.8, 5.5, 50.8];
                break;
            case 'curry':
                var p2stat=[13, 37.9, 30.5, 11.2, 23.9, 46.6, 4.4, 12.1, 36.3];
                break;
            case 'fox':
                var p2stat=[7, 38.5, 27.4, 10.0, 23.6, 42.4, 3.0, 9.0, 33.3];
                break;
            case 'giannis':
                var p2stat=[3, 30.5, 23.3, 9.3, 17.7, 52.8, 0.0, 1.0, 0.0];
                break;
            case 'harden':
                var p2stat=[11, 38.8, 20.3, 6.5, 16.6, 39.3, 2.8, 7.5, 37.8];
                break;
            case 'jimmy':
                var p2stat=[64, 33.4, 22.9, 7.5, 13.9, 53.9, 0.6, 1.6, 35.0];
                break;
            case 'joel':
                var p2stat=[9, 37.4, 23.7, 7.3, 17.0, 43.1, 0.6, 3.1, 17.9];
                break;
            case 'lebron':
                var p2stat=[55, 35.5, 28.9,  11.1, 22.2, 50.0, 2.2, 6.9, 32.1];
                break;
            case 'trae':
                var p2stat=[74, 36.4, 20.5, 8.3, 17.8, 46.4, 1.8, 5.2, 34.4];
                break;
        }

        // 여기에 선택된 이미지의 data-stat 값을 이용하여 Google Chart API를 사용한 막대 그래프 생성 코드를 추가하세요.
        // 막대 그래프를 생성하는 방법은 Google Chart API 문서를 참조하시면 도움이 됩니다.
        console.log(p1stat);
        console.log(p2stat);

        var data = google.visualization.arrayToDataTable([
            ['Genre', player1, player2, { role: 'style' } ],
            ['GP', p1stat[0], p2stat[0], 'color:  orange'],
            ['MIN', p1stat[1], p2stat[1], 'color: orange '],
            ['PTS', p1stat[2], p2stat[2], 'color: orange '],
            ['FGM', p1stat[3], p2stat[3], 'color: orange '],
            ['FGA', p1stat[4], p2stat[4], 'color: orange '],
            ['FG%', p1stat[5], p2stat[5], 'color: orange '],
            ['3PM', p1stat[6], p2stat[6], 'color: orange '],
            ['3PA', p1stat[7], p2stat[7], 'color: orange '],
            ['3P%', p1stat[8], p2stat[8], 'color: orange ']
          ]);
    // 	GP	MIN	PTS	FGM	FGA	FG%	3PM	3PA	3P%
        ...
        var chart = new google.visualization.BarChart(document.getElementById("charbox"));
        chart.draw(data, options);
      } else {
